...
def analyze_text(filename):
    ...
    Returns: A tuple where the first elements is the number of lines in
    ...
    with open(filename, 'r') as f:
        for line in f:
            lines += 1
            chars += len(line)
    return (lines, chars)#sum(1 for _ in f)
...
class TextAnalysisTests(unittest.TestCase):
    ...
    def tearDown(self):
        """Fixture taht deletes the files used by the test methods."""
        try:
    ...
    def test_character_count(self):
        ...
        self.assertEqual(analyze_text(self.filename)[1], 70)
